...
class ThreeD:
    ...
    __slots__ = (
        ...
        '_particles',
        '_field_surface',
        '_field_z0', '_field_height_scale', '_field_X', '_field_Y',
        '_field_cmap', '_particle_cmap', '_phase_cmap',
        '_color_mode', '_last_color_mode',
    )

    def __init__(self, grid_size: tuple[int, int, int], ax: object) -> None:
        ...
        ax.set(xlim=(0, gx), ylim=(0, gy), zlim=(0, gz), xlabel="X", ylabel="Y", zlabel="Z")
        ...
        self._field_X, self._field_Y = np.meshgrid(np.arange(gx), np.arange(gy), indexing="ij")
    ...
    def update(self, state: TensorDict) -> list[object]:
        ...
        pos = positions.detach().cpu().numpy() if hasattr(positions, 'detach') else np.asarray(positions)
        n = int(len(pos))
        ...
        if n == 0:
            ...
            elif heat is not None and heat.size == n and masses is not None and masses.size == n and (c_vf is not None and c_vf > 0.0):
            ...
            if particle_energy is not None and particle_energy.size == n:
                ...
                if vel_s is not None:
                    vel_s = vel_s.detach().cpu().numpy() if hasattr(vel_s, "detach") else np.asarray(vel_s)
            ...
            self._particles._offsets3d = (pos_plot[:, 0], pos_plot[:, 1], pos_plot[:, 2])
        ...
        if field2d is None:
            ...
            if gravity is not None:
                g = gravity.detach().cpu().numpy() if hasattr(gravity, 'detach') else np.asarray(gravity)
                if g.ndim == 3:
                    g = g[:, :, g.shape[2] // 2]
                if g.size > 0:
                    field2d = g
    ...
    def _render_field(self, g: np.ndarray) -> None:
        ...
        self._field_surface = self.ax.plot_surface(
            self._field_X, self._field_Y, Z, rstride=1, cstride=1, facecolors=facecolors,
            linewidth=0.0, antialiased=False, shade=False, zorder=0)

    def _coupling_or_support_field_xy(self, state: TensorDict) -> tuple[Optional[np.ndarray], str]:
        ...
        aidx = aidx.detach().cpu().numpy() if hasattr(aidx, "detach") else np.asarray(aidx)
        ...
        pos_max = float(np.nanmax(pos)) if pos.size else 0.0
        dom_max = float(np.max(domain))
        pos_phys = pos if (dom_max > 0.0 and pos_max <= (1.01 * dom_max)) else (pos * dx)
