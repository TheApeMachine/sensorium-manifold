...
% ============================================================================
\begin{abstract}
We present the \emph{Sensorium Manifold}, a physics-native computing substrate for learning and inference. The motivating claim is pragmatic: modern machine learning already treats biology as useful engineering metaphor (e.g., the perceptron), so using physics as a computational metaphor is a natural next step, not a categorical break. We frame intelligence operationally as \emph{information management}: introducing data into a high-dimensional dynamical system and driving it toward usable structure.

The system couples particles through a hydrodynamic wave field $\Psi(\omega)$ with interference, superposition, and tunneling. Under observation-driven forcing, controlled dissipation, and homeostatic regulation, the dynamics self-organize into persistent wells in $|\Psi(\omega)|$ that store and bind patterns. No loss function or gradient descent is required; structure emerges because the governing dynamics favor stable attractors. This yields a \emph{Holographic Content Addressable Memory} in which partial cues retrieve complete patterns, and recasts generation as global relaxation (a boundary-value solve) rather than strictly serial autoregression.

The same dynamics are modality-agnostic under spectral isomorphism, enabling one substrate for text, image, and audio without modality-specific fusion objectives. The current draft reports mechanism-level multimodal and sequence results while medium- and full-scale experiments are actively being run; this is a working manuscript, not a publication claim until those scale studies are complete. We use AI as a demanding test domain rather than the endpoint itself. The central contribution is evidence that thermodynamic-wave dynamics can function as a practical information-processing system; the open question is scale.
\end{abstract}
...
Universal Tokenizer & Embedding Layer & Deterministic byte+index composite IDs; no training. \\
...
Most modern machine learning systems are trained as parametric function approximators with explicit objectives and gradient-based optimization. This paper studies a different computational route: learning and inference as state evolution in an open physical dynamical system.

Our framing is operational. We treat intelligence tasks as information-management problems: observations perturb a high-dimensional substrate, and useful behavior corresponds to the emergence of stable, reusable structure under those perturbations. The question is not whether this system is biologically faithful, but whether physics-governed dynamics can provide a practical computational mechanism.

To test this, we implement the Sensorium Manifold as a GPU-executable substrate combining particle-grid thermodynamics, a hydrodynamic frequency field $\Psi(\omega)$, and symplectic integration. The resulting system is designed to be falsifiable: it should either form useful associative structure under streaming observations, or fail clearly on measurable tasks.

We evaluate on small-scale but diverse workloads (byte prediction, rule-shift adaptation, audio/image reconstruction, and cross-modal coupling). The current evidence is therefore proof-of-mechanism rather than proof-of-scale.

\paragraph{Physics as a computing paradigm.}
This work is \emph{physics as a computing paradigm}, not a simulation of a specific laboratory system. We use established equations (compressible Navier--Stokes, Gross--Pitaevskii-style wave dynamics, Planck-informed equilibration) as algorithmic structure for representation and inference in an abstract computational state space. Hardware realizability is outside scope.

\paragraph{System overview.}
The Sensorium Manifold is a learning system where:
\begin{itemize}
  \item \textbf{Particles} represent active units that carry mass, momentum, and internal energy in continuous space
  \item \textbf{Hydrodynamic $\omega$-field} is a complex wavefunction $\Psi(\omega)$ whose persistent wells encode reusable structure
  \item \textbf{Crystallization} is the stabilization of peaks in $|\Psi(\omega)|$ under observation-driven forcing and controlled dissipation
  \item \textbf{Heat} stores entropic uncertainty and supports exploration through diffusion and exchange
  \item \textbf{Homeostasis} regulates global activity through adaptive energy baselines to prevent runaway excitation or collapse
\end{itemize}
...
\begin{enumerate}
  ...
  \item The Universal Tokenizer: modality-agnostic encoding via deterministic composite IDs (\Cref{sec:tokenizer})
...
We map raw data to the manifold using a deterministic composite identifier:

\begin{equation}
\text{ID} = (\text{Index} \ll 8)\,|\,\text{Byte}
\label{eq:token_id}
\end{equation}

where $\text{Byte} \in [0, 255]$ is the raw data and $\text{Index}$ is the sequence position (or spatial coordinate). This mapping is collision-free for distinct $(\text{Byte},\text{Index})$ pairs in the representable integer range. In the current implementation, intrinsic frequency $\omega$ is derived from byte value; the composite ID provides deterministic identity.
...
\begin{enumerate}
  ...
  \item \textbf{Identity match}: $(b_a,s_a)=(b_b,s_b)$ (equivalently, identical token IDs from \Cref{eq:token_id}).
\end{enumerate}
...
Consider the sequence ``AB AB.'' Tokens `A' at positions 0 and 3 receive the same token ID when position is wrapped by segment size so both map to relative position 0: $\text{ID}(\texttt{0x41},0)=\text{ID}(\texttt{0x41},0)$. In the spatial layer, the $\omega$-field coupling and gravity pull both `A' particles toward the same geometric coordinate, because their frequency-space support overlaps. When they arrive in the same cell, the merge operator (\Cref{sec:inelastic_collision}) absorbs one into the other. The result: a single particle `A' with mass 2. The system has \emph{physically folded} the sequence ``AB AB'' onto itself, discovering that it is ``AB'' repeated. Mass becomes frequency---the confidence that a pattern exists at that location.
...
\begin{itemize}
    \item \textbf{Text}: $\text{ID}(\text{'H'}, 0) \to \text{ID}(\text{'e'}, 1) \dots$
    \item \textbf{Image}: $\text{ID}(\text{0xFF}, 0) \to \text{ID}(\text{0x00}, 1) \dots$
\end{itemize}
...
To validate that repeated token IDs create useful structure, we train on 7 text patterns (e.g., ``The cat sat.'') with segment size 16. The character `T' at position 0 receives the same token ID across all patterns, creating the thermodynamic trie.

\textbf{Result}: The system achieves 92.4\% overall accuracy---99.3\% on deterministic paths (single valid continuation) and 55.8\% at branch points (multiple valid continuations). This demonstrates that the trie structure emerges naturally from repeated byte+position IDs, enabling prediction without gradient-based training. Full details in Appendix~\ref{sec:next_byte_prediction}.
...
Cross-modal transduction works because the Universal Tokenizer assigns deterministic token identity to all particles regardless of modality. A text token, an audio sample, and an image pixel all become particles with IDs from (byte, index) and oscillator frequencies derived from byte value.
...
\begin{itemize}
    \item An audio sample of a cat meowing may map to frequencies near the text ``meow''.
...
We validate the framework across five experiments that probe different aspects of the manifold's behavior: (1) the ``collision is compression'' mechanism where repeated byte+position IDs create hierarchical trie structure, (2) online adaptation to distributional shifts, (3) audio waveform reconstruction, (4) multi-source audio separation, and (5) native image handling.
These are mechanism-validation studies. We do not present them as final scale benchmarks.

\subsection{Evaluation Protocol and Anti-Toy-Scale Controls}
\label{sec:eval_protocol}

To make scale-related claims reviewable, we use the following controls for new benchmark runs:
\begin{enumerate}
  \item \textbf{Strict data partitioning}: train/validation/test splits are fixed in advance, with no overlap by sample identity or contiguous windows for sequence data.
  \item \textbf{Compute-matched baselines}: comparisons are reported at matched training FLOPs and matched wall-clock budgets, not parameter count alone.
  \item \textbf{Multi-seed statistics}: each condition is run across multiple random seeds, with mean, standard deviation, and confidence intervals reported.
  \item \textbf{Independent scaling sweeps}: sequence length $N$, $\omega$-lattice size $k$, particle count, and grid size are swept one factor at a time before joint sweeps.
  \item \textbf{Negative controls}: shuffled-label or shuffled-position controls are reported to verify that measured gains are not artifacts of the tokenizer or evaluation pipeline.
\end{enumerate}

\begin{table}[!htbp]
  \centering
  \caption{Scale-validity criteria for core claims.}
  \label{tab:scale_validity}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{P{0.24\textwidth} P{0.42\textwidth} P{0.27\textwidth}}
    \toprule
    \textbf{Claim} & \textbf{Minimum evidence required} & \textbf{Failure condition} \\
    \midrule
    Length-insensitive step cost & Fit $t_{\text{step}}(N)=aN^\alpha$ on $N\in\{1\mathrm{k},4\mathrm{k},16\mathrm{k},64\mathrm{k}\}$ at fixed $k$ and hardware; report $\alpha$ with CI. & Lower CI bound for $\alpha$ remains clearly above zero. \\
    Competitive byte/text modeling & On text8/enwik8-style corpora, report bits-per-byte and throughput at matched FLOPs against n-gram + compact transformer/state-space baselines. & Worse quality-efficiency frontier than all matched baselines. \\
    Continual adaptation & Under repeated and adversarial rule shifts, report recovery half-life, forgetting, and backward transfer across seeds. & Slow recovery or persistent forgetting relative to baselines. \\
    Cross-modal reuse & One shared manifold with no modality-pair-specific fusion objective; report Recall@K and reconstruction metrics. & Gains appear only in single-modality settings or collapse out-of-distribution. \\
    Mechanism necessity & Ablate homeostasis, Planck exchange, inelastic merge, and $\omega$-coupling independently. & Comparable performance after removing proposed core mechanisms. \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Tokenizer-ID Collision Compression}
\label{sec:image_collision}

This experiment validates the core insight of the Universal Tokenizer: repeated byte+position IDs are not errors but \emph{compression}. When multiple data streams share prefixes, they coalesce into the same particle identities, creating a thermodynamic trie that naturally clusters related patterns.

\paragraph{Method.}
We process image-sized byte streams using the Universal Tokenizer, varying shared-prefix overlap via the dataset collision-rate control. At each setting, we measure: (1) spatial clustering of particles with identical token IDs, (2) energy accumulation in colliding particles, and (3) compression metrics (entropy, compression ratio).
...
\paragraph{Primary Evidence (Real-World Stream).}
For the strongest non-toy demonstration of collision folding in this draft, we pin the \texttt{medium\_wikitext} run in the main body (not the appendix). This uses a real text stream (WikiText-2 train split) and is automatically included whenever experiment artifacts exist.

\begin{table}[!htbp]
  \centering
  \caption{Pinned real-world collision-folding run and conditions (automatically paired with generated artifacts).}
  \label{tab:collision_wikitext_conditions}
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{ll}
    \toprule
    \textbf{Condition} & \textbf{Value} \\
    \midrule
    Scenario & \texttt{collisionexperiment\_medium\_wikitext} \\
    Dataset & WikiText-2 raw v1 (train stream) \\
    Max samples & 512 \\
    Grid size & $64\times64\times64$ \\
    Step budget & max 12 (min 4) \\
    Tokenizer budget & max\_tokens = 65536 \\
    Runtime backend & MPS (when available) \\
    Recall test & Next-edge retrieval from transition trie (Recall@1/Recall@3/MRR) \\
    Evidence artifacts & \texttt{...\_folding\_evidence.pdf}, \texttt{...\_bifurcation.pdf} \\
    \bottomrule
  \end{tabular}
\end{table}

\IfFileExists{tables/CollisionExperiment_summary.tex}{%
  \input{tables/CollisionExperiment_summary.tex}
}{%
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{0.8cm}\textit{Run \texttt{python -m sensorium.experiments --experiment collision} to generate \texttt{CollisionExperiment\_summary.tex}}\vspace{0.8cm}}}
}

\begin{figure}[!htbp]
  \centering
  \IfFileExists{figures/collisionexperiment_medium_wikitext_folding_evidence.pdf}{%
    \includegraphics[width=0.95\textwidth]{figures/collisionexperiment_medium_wikitext_folding_evidence.pdf}
  }{%
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}\textit{Run \texttt{make experiments} to generate \texttt{collisionexperiment\_medium\_wikitext\_folding\_evidence.pdf}}\vspace{2cm}}}
  }
  \caption{Collision folding and recall evidence on the real-stream WikiText scenario. Left: fold concentration vs path top-1 concentration. Middle: fold participation ratio and fold entropy. Right: explicit trie recall test (Recall@1/Recall@3/MRR) showing recoverability of next-step information from the learned transition structure.}
  \label{fig:collision_wikitext_folding_evidence}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \IfFileExists{figures/collisionexperiment_medium_wikitext_bifurcation.pdf}{%
    \includegraphics[width=0.82\textwidth]{figures/collisionexperiment_medium_wikitext_bifurcation.pdf}
  }{%
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{2cm}\textit{Run \texttt{make experiments} to generate \texttt{collisionexperiment\_medium\_wikitext\_bifurcation.pdf}}\vspace{2cm}}}
  }
  \caption{Top transition-edge bifurcation sketch for the same real-stream run, showing branching structure in the observed transition graph.}
  \label{fig:collision_wikitext_bifurcation}
\end{figure}

\IfFileExists{tables/image_collision_summary.tex}{%
...
\begin{figure}[!htbp]
  ...
  \caption{Repeated byte+position IDs create compression in a thermodynamic trie. As collision rate increases (left to right), particles with identical token IDs cluster in 3D space, energy accumulates in shared particles, and compression metrics improve. The visualization demonstrates that deterministic ID coalescence naturally creates hierarchical structure.}
...
We evaluate adaptation to distributional shifts using a scaled rule-shift matrix designed to address reviewer-grade validity concerns: multiple shift families, repeated seeds, multi-phase curricula, and explicit recall metrics.

\paragraph{Method.}
The upgraded study runs a scenario matrix across synthetic control, natural-language style shift, three-phase continual shift, and a real-world-like WikiText-style condition. Each scenario is repeated over multiple seeds with explicit conditions reported in \Cref{tab:rule_shift} (seeds, phases, repetitions, and segment length). Evaluation remains strictly online: at each checkpoint, next-byte prediction uses only prefix data seen so far.

\paragraph{Results.}
\Cref{tab:rule_shift} reports mean$\pm$std performance across seeds and conditions. The key pattern is consistent: each shift induces a measurable drop, followed by recovery without gradient-based retraining, and with retained recall quality (Final@1, Final@3, MRR). \Cref{fig:rule_shift} summarizes the full matrix; \Cref{fig:rule_shift_realworld} isolates the strongest real-world-like scenario for direct inspection.
...
\begin{figure}[!htbp]
  ...
  \caption{Scaled rule-shift matrix. \textbf{(A)} Top-1 adaptation trajectories by scenario. \textbf{(B)} Final recall quality (Final@1, Final@3, MRR). \textbf{(C)} Shock-vs-recovery summary (worst drop and recovery repetitions). \textbf{(D)} Strongest-scenario recall dynamics.}
  \label{fig:rule_shift}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \IfFileExists{figures/rule_shift_realworld.png}{%
    \includegraphics[width=0.86\textwidth]{figures/rule_shift_realworld.png}
  }{%
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{2cm}\textit{Real-world detail figure will be generated by \texttt{make paper}}\vspace{2cm}}}
  }
  \caption{Real-world-like rule-shift focus run: Top-1, Top-3, and MRR over repetitions. This panel is pinned in the main section (not appendix) because it represents the highest-fidelity scenario in the matrix.}
  \label{fig:rule_shift_realworld}
\end{figure}
...
\IfFileExists{tables/audio_gen_summary.tex}{%
...
\paragraph{Method.}
The mixed audio file is tokenized byte-by-byte using the Universal Tokenizer, creating particles with deterministic byte+position IDs and byte-derived frequencies. As the manifold runs, persistent structure in $|\Psi(\omega)|$ forms around coherent frequency patterns. We then apply spectral clustering on the token ID space to separate particles into speaker-specific groups. Each group is decoded back to byte values (equivalently, low 8 bits), producing separated audio streams.
...
\paragraph{Method.}
Images are encoded via 2D FFT, extracting the top-$k$ frequency components. Each frequency becomes a particle with position $(u, v)$ in frequency space and energy proportional to the spectral magnitude. Text tokens are encoded using the same deterministic byte+index tokenizer. Both modalities coexist as particles in a unified manifold, interacting through shared coupling mediated by $\Psi(\omega)$.
\par
...
We provide preliminary scaling evidence for the manifold, focusing on step-cost behavior and $\omega$-field structure dynamics in the tested regime.
...
We measure per-step latency across sequence lengths from 500 to 8,000 tokens. \Cref{tab:scaling} shows latency near $\approx$3 ms/step with 11\% coefficient of variation on fixed hardware. This is consistent with step cost being dominated by $\omega$-mode count $k$ (field resolution) in this range; broader claims are deferred to the protocol in \Cref{sec:eval_protocol}.
...
\IfFileExists{tables/ablation.tex}{%
  ...
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{0.8cm}\textit{Ablation table will be generated by \texttt{make paper}}\vspace{0.8cm}}}
}
...
CLIP \citep{radford2021learning} and Flamingo \citep{alayrac2022flamingo} require explicit cross-modal coupling mechanisms. Unified-IO 2 \citep{lu2024unifiedio2} tokenizes all modalities into a shared space. Our Universal Tokenizer achieves modality-agnostic encoding through deterministic byte+index composite IDs, with cross-modal coupling emerging from shared $\omega$-space hydrodynamic dynamics.
...
We reiterate the core framing from \Cref{sec:introduction}: this is physics-inspired computation, not a claim of literal physical simulation fidelity. The equations define algorithmic update rules in an abstract state space. The appropriate evaluation question is therefore empirical: does this substrate provide useful learning behavior under controlled benchmarks?

\subsection{What Current Evidence Supports}

\begin{enumerate}
  \item \textbf{Mechanistic viability}: The coupled spatial and $\omega$-field dynamics run stably and produce persistent structure under observation-driven forcing.
  \item \textbf{Gradient-free adaptation at small scale}: Across the reported tasks, useful behavior emerges without optimizing a learned loss surface.
  \item \textbf{Online response to distribution shift}: In the rule-shift experiment, performance recovers quickly after abrupt context change.
  \item \textbf{Shared substrate across modalities}: The same dynamics process byte/text, image-frequency, and audio-byte inputs in one framework.
  \item \textbf{Length-insensitive step cost in tested regimes}: Within the measured range, per-step latency is primarily controlled by field resolution and particle workload, not sequence length alone.
\end{enumerate}

\subsection{What Remains Open}

\begin{enumerate}
  \item \textbf{Scale competitiveness}: We have not shown parity with large autoregressive models on standard language or multimodal leaderboards.
  \item \textbf{Robustness across seeds and regimes}: Several experiments are still single-setting demonstrations rather than broad statistical studies.
  \item \textbf{Capacity limits}: The relationship between $\omega$-lattice resolution, memory capacity, and retrieval error needs formal characterization.
  \item \textbf{Failure modes}: We need clearer stress tests for catastrophic interference, saturation, and out-of-distribution behavior.
...
To make the central claims falsifiable for an ML audience, we propose the following next-phase protocol:
...
\begin{enumerate}
  ...
  \item \textbf{Continual rule-shift benchmark}: extend the current shift task to multi-phase curricula with recurring and adversarial shifts. Report adaptation half-life, backward transfer, and forgetting.
  ...
  \item \textbf{Scaling decomposition}: sweep sequence length $N$, field resolution $k$, particle count, and grid size independently. Fit empirical scaling exponents and report breakpoints where memory bandwidth or synchronization dominates.
  \item \textbf{Ablation and counterfactual tests}: remove one mechanism at a time (homeostasis, Planck exchange, inelastic merge, $\omega$-field coupling) and quantify performance deltas with confidence intervals.
\end{enumerate}

For each benchmark, report mean and variance across multiple seeds, matched compute budgets, and explicit failure cases.

\subsection{How We Handle the ``Toy-Scale'' Objection}

The strongest critique of this paper is that mechanism-level demonstrations may not survive larger regimes. We treat this as a valid default position, not a misunderstanding. Concretely, any scale-sensitive claim in this paper should be considered provisional unless it satisfies the criteria in \Cref{tab:scale_validity}. This shifts the burden from narrative plausibility to quantitative acceptance tests.

Two implications follow. First, negative outcomes are informative: if the system fails these criteria, the correct conclusion is that the current formulation is not a scalable alternative. Second, positive outcomes must be compute-accounted: gains that disappear under matched FLOPs or wall-clock budgets do not count as evidence for a superior paradigm.

\subsection{Interpreting Cross-Modal ``Horizontal''}

The claim that ``horizontal'' shares structure across text, audio, and vision should be read as a testable representation hypothesis: modality-specific signals that co-activate consistently can reinforce overlapping regions in $\Psi(\omega)$, enabling transfer through shared dynamical wells. The current experiments are consistent with this hypothesis, but larger and more diverse datasets are required before making strong generalization claims.
...
We presented the Sensorium Manifold as a physics-native substrate for learning and inference, with one shared dynamical framework for text, image, and audio representations. Across the reported experiments, the system demonstrates mechanism-level behaviors that motivate the approach: stable $\omega$-field structure, gradient-free adaptation, and modality-agnostic processing under a common state evolution.

At the same time, this draft does not claim large-scale superiority over modern sequence models. The central unresolved question is scale. For that reason, we define explicit acceptance criteria (\Cref{tab:scale_validity}) for latency scaling, benchmark competitiveness, continual adaptation robustness, cross-modal reuse, and mechanism necessity. These criteria are intended to be easy to fail and hard to game.

If those tests are passed under matched compute, the work supports a substantive alternative to optimization-centric architectures. If they are not, the contribution remains a useful negative result about the limits of this physical formulation. In either case, the paper advances a falsifiable research program rather than a closed claim.
...
\begin{figure}[!htbp]
  ...
  \caption{Synthetic trie collision sweep. As collision rate increases, spatial clustering increases and compression ratio decreases, demonstrating that repeated byte+position IDs act as compression.}
...
This experiment demonstrates how the Universal Tokenizer creates a \emph{thermodynamic trie}---a branching structure that emerges from controlled byte+position ID overlap. The key insight is that repeated IDs are not errors but \emph{compression}: identical (byte, position) pairs across different occurrences coalesce into the same particle identity, allowing the manifold to learn statistical patterns.
...
\begin{enumerate}
\item \textbf{Tokenize}: Each byte at position $p$ is mapped to token ID $t = (p \ll 8)\,|\,b$.
...
\IfFileExists{tables/next_token_summary.tex}{%
...
\IfFileExists{tables/mnist_bytes_summary.tex}{%
...
\begin{figure}[!htbp]
  ...
  \caption{Kernel MNIST confusion matrix (Universal Tokenizer byte+index pixel stream).}
...
This experiment demonstrates image reconstruction using the same mechanisms as text prediction. Pixels are treated as bytes and paired with their 2D position to create deterministic token IDs in a thermodynamic trie. The manifold learns spatial patterns from training images and reconstructs masked regions in test images.

\paragraph{Method.}
Each pixel at position $(x, y)$ is mapped to a token ID: $t = ((y \cdot 28 + x) \ll 8)\,|\,p$, where $p$ is the pixel intensity byte. For reconstruction, we use dual-domain inference: geometric locality (neighboring pixels) provides spatial smoothness, while $\omega$-space wells in $\Psi(\omega)$ provide pattern completion from similar images in the training set.
...
\IfFileExists{tables/text_diffusion_summary.tex}{%
...
\section{Review-Ready Benchmark Templates}
\label{app:review_templates}

The following templates are intended for the next revision cycle, where scale-sensitive claims are evaluated under matched-compute conditions.

\begin{table}[!htbp]
  \centering
  \caption{Long-context byte/text benchmark template (fill with measured values).}
  \label{tab:template_text_scale}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{P{0.11\textwidth} P{0.09\textwidth} P{0.10\textwidth} P{0.10\textwidth} P{0.10\textwidth} P{0.10\textwidth} P{0.10\textwidth} P{0.10\textwidth}}
    \toprule
    \textbf{Model} & \textbf{Dataset} & \textbf{Context $N$} & \textbf{Train FLOPs} & \textbf{Wall-clock (h)} & \textbf{Bits/byte} & \textbf{Tokens/s} & \textbf{ms/step} \\
    \midrule
    Sensorium & text8 &  &  &  &  &  &  \\
    Sensorium & enwik8 &  &  &  &  &  &  \\
    N-gram baseline & text8 &  &  &  &  &  &  \\
    Compact Transformer & text8 &  &  &  &  &  &  \\
    Compact SSM/RNN & text8 &  &  &  &  &  &  \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  \centering
  \caption{Continual adaptation template (distribution-shift protocol).}
  \label{tab:template_adaptation}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{P{0.15\textwidth} P{0.13\textwidth} P{0.14\textwidth} P{0.13\textwidth} P{0.14\textwidth} P{0.10\textwidth}}
    \toprule
    \textbf{Model} & \textbf{Shift Type} & \textbf{Recovery Half-life} & \textbf{Forgetting ($\downarrow$)} & \textbf{Backward Transfer} & \textbf{Seeds} \\
    \midrule
    Sensorium & periodic $\rightarrow$ reversed &  &  &  &  \\
    Sensorium & adversarial shifts &  &  &  &  \\
    Compact Transformer & periodic $\rightarrow$ reversed &  &  &  &  \\
    Compact SSM/RNN & periodic $\rightarrow$ reversed &  &  &  &  \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  \centering
  \caption{Scaling decomposition template (single-factor sweeps + fitted exponents).}
  \label{tab:template_scaling}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{P{0.14\textwidth} P{0.10\textwidth} P{0.12\textwidth} P{0.12\textwidth} P{0.14\textwidth} P{0.18\textwidth}}
    \toprule
    \textbf{Sweep Variable} & \textbf{Range} & \textbf{Held Fixed} & \textbf{Fit Form} & \textbf{Exponent (CI)} & \textbf{Primary Bottleneck} \\
    \midrule
    Sequence length $N$ &  & $k$, grid, hardware & $t_{\text{step}}=aN^\alpha$ &  &  \\
    Field size $k$ &  & $N$, grid, hardware & $t_{\text{step}}=bk^\beta$ &  &  \\
    Particle count &  & $N$, $k$, hardware & $t_{\text{step}}=cn^\gamma$ &  &  \\
    Grid resolution &  & $N$, $k$, hardware & $t_{\text{step}}=dg^\delta$ &  &  \\
    \bottomrule
  \end{tabular}
\end{table}
